...
//thread conditions and stuff
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
int done = 0;
pthread_cond_t cond;
int p_i = 0;
//pthread_t item *current;

//array of subscriber heads and tails
struct item **sub_head = NULL;
struct item **sub_tail = NULL;

//wrapper and stuff for wrappper
pub_init_t *t_pubs;
sub_init_t *t_subs;
char **t_pubs_arg;
char **t_pubs_arg;
...
void *publisher(void *p){
    t_pubs[p_i](t_pubs_arg[p_i], simple_publish);
}
...
int main(int argc, char **argv)
{
    ...
    t_pubs = malloc(sizeof(*pubs) * (argc/2));
    t_subs = malloc(sizeof(*subs) * (argc/2));
    t_pubs = pubs;
    t_subs = subs;


    //initialize the sub_head and tail list by setting individual head and tail for each subscriber
    for(int i =0; i<sub_count; i++){
        sub_head[i] = head;
        sub_tail[i] = tail;
    }

    pthread_t pub_thread[pub_count];
    pthread_t sub_threads[sub_count];

    // do all the pubs first (this might fail if the pubs are also subs...)
    for (int i = 0; i < pub_count; i++) {
        pthread_create(&pub_thread[i], NULL, publisher, (void *)i);
        //pthread_create()pubs[i](pubs_arg[i], simple_publish);
    }
